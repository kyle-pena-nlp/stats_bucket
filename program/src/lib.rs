...
fn deserialize_into_float_array(input: &[u8]) -> Result<&[f32], ProgramError> {
    bytemuck::try_cast_slice(input).map_err(|_| ProgramError::InvalidAccountData)
}

// Main program logic
fn process_instruction(
    _program_id: &Pubkey,       // Program ID
    _accounts: &[AccountInfo],  // Account info array
    instruction_data: &[u8],    // Instruction data
) -> ProgramResult {
    msg!("Hello, Solana!");

    let ys = deserialize_into_float_array(instruction_data).unwrap(); 

    let mut bucket = StatsBucket::init_empty();
